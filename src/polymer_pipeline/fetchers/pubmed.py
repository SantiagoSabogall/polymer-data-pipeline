...
def fetch_pubmed(query: str, max_results: int = 100) -> list:
    ...
    with make_session() as session:
        try:
            resp = session.get(base_url + "esearch.fcgi", params=search_params, timeout=15)
        ...
        try:
            resp = session.get(base_url + "efetch.fcgi", params=fetch_params, timeout=15)
            resp.raise_for_status()
