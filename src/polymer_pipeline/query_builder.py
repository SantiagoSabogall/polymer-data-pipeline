...
def _crossref_term(term: str) -> str:
    ...
    Si el término ya viene entrecomillado por el usuario, se respeta
    (coincidencia exacta) y no se agrega wildcard.
    """
...
def _pubmed_term(term: str) -> str:
    ...
    Si el término ya viene entrecomillado por el usuario, se agrega el campo
    sin duplicar comillas.
    """
    if term.startswith('"') and term.endswith('"'):
